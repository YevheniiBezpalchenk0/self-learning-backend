...
public class AuthenticationService {
    ...
    public void register(RegisterRequest request, HttpServletResponse response) throws IOException {
        if (repository.existsByEmail(request.getEmail())) {
            ...
            var errorResponse = Map.of("message", "Email is already in use");
        ...
        var authResponse = AuthenticationResponse.builder().accessToken(jwtToken).role(user.getRole()).build();
    ...
    public void authenticate(AuthenticationRequest request, HttpServletResponse response) throws IOException {
        ...
        System.out.println(" Start response creating");
        var authResponse = AuthenticationResponse.builder().accessToken(jwtToken).role(user.getRole()).build();
        new ObjectMapper().writeValue(response.getOutputStream(), authResponse);
    }
    ...
    public void refreshToken(String refreshToken, HttpServletResponse response) throws IOException {
        ...
        if (userEmail != null) {
            var user = this.repository.findByEmail(userEmail).orElseThrow(() -> new RuntimeException("User not found"));
            ...
            if (jwtService.isTokenValid(refreshToken, user)) {
                ...
                var authResponse = AuthenticationResponse.builder().accessToken(accessToken).role(user.getRole()).build();
